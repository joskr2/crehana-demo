...
const Card: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      if (data) {
        if (data && data.searchByName && data.searchByName[0]) {
          return (await data) && data.searchByName && data.searchByName[0];
        } else if (
          data &&
          data.searchByAlpha2Code &&
          data.searchByAlpha2Code[0]
        ) {
          return (
            (await data) &&
            data.searchByAlpha2Code &&
  ...
  return (
    <>
      {infoDetail && infoDetail.name ? (
        <div className="p-8 max-w-sm rounded overflow-hidden shadow-lg">
          <img
            className="w-full"
            src={`${infoDetail && infoDetail.flag && infoDetail.flag.svgFile}`}
            alt="country_flag"
          />
          <div className="px-6 py-4">
            ...
            <p className="text-gray-700 text-base">
              ...
              {infoDetail &&
                infoDetail.currencies &&
                infoDetail.currencies[0].name}
            </p>
            <button
                    ...
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                      ...
                      <div className="relative p-6 flex-auto">
                        <p className="text-gray-700 text-base">
                          ...
                          {infoDetail &&
                            infoDetail.flag &&
                            infoDetail.flag.emoji}
                        </p>
                        <p className="text-gray-700 text-base">
                          <strong>Código: </strong>{" "}
                          {infoDetail && infoDetail.numericCode}
                        </p>
                        <p className="text-gray-700 text-base">
                          <strong>Capital: </strong>{" "}
                          {infoDetail && infoDetail.capital}
                        </p>
                        <p className="text-gray-700 text-base">
                          <strong>
                            Moneda{" "}
                            {`(${
                              infoDetail &&
                              infoDetail.currencies &&
                              infoDetail.currencies[0].symbol
                            })`}{" "}
                            :{" "}
                          </strong>
                          {infoDetail &&
                        ...
                        <p className="text-gray-700 text-base">
                          <strong>Área: </strong>{" "}
                          {infoDetail && infoDetail.area} millones km²
                        </p>
                        <p className="text-gray-700 text-base">
                          <strong>Densidad Poblacional: </strong>{" "}
                          {infoDetail && infoDetail.populationDensity} por km²
                        </p>
                        <p className="text-gray-700 text-base">
                          <strong>Nombre Nativo: </strong>{" "}
                          {infoDetail && infoDetail.nativeName}
                        </p>
                        <p className="text-gray-700 text-base">
                          <strong>Población: </strong>{" "}
                          {infoDetail && infoDetail.population} millones
                        </p>
                      </div>
                      {/*footer*/}
